...
`locationrelay` is a single-route ingest service. Its security posture comes from
having almost no surface area, then hardening the little that remains.

## Trust boundaries
- The phone (Overland) holds a shared bearer token. The token is the only thing
  authenticating a writer.
